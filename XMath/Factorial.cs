...
namespace CSBoost
{
    ...
    {
        public const int max_factorial = 171;
        ...
        public static double factorial(uint i)
        {
            if (i <= max_factorial) return factorials()[i];
            double result = gamma(i + 1);
            if (result > double.MaxValue) return result; 
            return Math.Floor(result + 0.5);
        }

        public static double double_factorial(uint i)
        {
            if ((i & 1) > 0)
            {
                // odd i:
                ...
                {
                    uint n = (i - 1) / 2;
                    return Math.Ceiling(factorials()[i] / (ldexp(1.0, (int)n) * factorials()[n]) - 0.5);
                }
                double result = gamma(i / 2 + 1) / Math.Sqrt(Math.PI);
                if (ldexp(double.MaxValue, -(int)(i + 1) / 2) > result) return Math.Ceiling(result * ldexp(1, (int)(i + 1) / 2) - 0.5);
            }
            else
            {
                // even i:
                ...
                if (ldexp(double.MaxValue, -(int)n) > result) return result * ldexp(1, (int)n);
            }
            //
        ...
        public static double rising_factorial(double x, int n)
        {
            if (x < 0)
            {
                //
                ...
                double result = ((n & 1) > 0 ? -1 : 1) * falling_factorial(-x, (uint)n);
                if (inv)
                    result = 1 / result;
                return result;
            }
            if (n == 0) return 1;
            return 1 / gamma_delta_ratio(x, n);
        }

        public static double falling_factorial(double x, uint n)
        {
            if (x == 0) return 0;
            if (x < 0) return ((n & 1) > 0 ? -1 : 1) * rising_factorial(-x, (int)n);
            if (n == 0) return 1;
            if (x < n - 1)
            {
                double xp1 = x + 1;
                uint n2 = (uint)(Math.Floor(xp1));
                if (n2 == xp1) return 0;
                double result = gamma_delta_ratio(xp1, -(double)n2);
                x -= n2;
                result *= x;
                ++n2;
                if (n2 < n) result *= falling_factorial(x - 1, n - n2);
                return result;
            }
            return gamma_delta_ratio(x + 1, -(double)n);
        }

        public static double binomial_coefficient(uint n, uint k)
        {
            if (k > n) throw new ArgumentException(string.Format("Binomial Coefficient: argument n must be >= argument k (got {0:G} and {1:G})", n, k));
            double result;
            ...
            else
            {
                ...
                if (k < n - k)
                    result = k * beta(k, n - k + 1);
                else
                    result = (n - k) * beta(k + 1, n - k);
                if (result == 0) throw new OverflowException("Binomial Coefficient: Arithmetic Overflow");
                result = 1.0 / result;
            }
